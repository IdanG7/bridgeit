...
Stackly is discoverable and installable by strangers on the internet: a stranger who has never heard of Stackly can land on a page, understand the product in under 60 seconds, follow a quickstart that works on a fresh Windows dev machine, and find the project via multiple independent channels (MCP directories, HN, Reddit, Twitter, search).
...
1. **README rewrite for public audience is shipped on `main`:**
   ...
   - Includes an animated GIF or embedded video showing the `stackly fix` loop end-to-end.
   - Quickstart section that a stranger can follow on a fresh Windows 10/11 machine: install prerequisites (Windows Debugging Tools, `uv`, Claude Code CLI), clone, `uv sync`, run `stackly doctor`, run `stackly serve`, try the sample `crash_app`.
   - "Why Stackly" section naming the competitive wedge (remote debug state + MCP + autonomous repair — no other tool combines all three).
...
2. **Landing page at `stackly.dev` is live:**
   ...
   - Page source is committed to the Stackly repo under `site/` (or a sibling repo, decided in PLAN.md) so the project owns its marketing surface.
...
3. **60-second demo video is recorded, edited, and uploaded:**
   ...
   - Shows: remote process crash on a test machine → `stackly fix` on dev machine → Claude Code diagnoses → patch written → build passes.
...
4. **Submitted to at least 4 MCP directories and all submissions tracked:**
   ...
   - Each submission includes: canonical name "Stackly", one-line description, install command, HTTP + stdio config examples, link to README + landing page + video, MIT license confirmation.
...
6. **Open-source announcement is consolidated:**
   ...
   - GitHub repo "About" section filled in: short description, homepage URL (stackly.dev), tags (mcp, claude-code, windows, debugging, cpp, crash-reporter).
...
7. **Discoverability sanity check passes:**
   - A fresh Google search for "Stackly MCP" or "stackly crash fix" surfaces the landing page or GitHub repo within the first two pages within 7 days of launch (measured manually, not blocking the phase exit — but set up Google Search Console so the site is indexable).
   - `robots.txt` + `sitemap.xml` on the landing page.
   - Repo has a clean `description` + `homepage` field set via `gh repo edit`.

## Non-goals (explicitly out of scope)

- **PyPI publish** — Phase 2c. The README + landing page reference "install from source" and document the path to `pip install stackly` coming in 2c. If PyPI ships during this phase as a side-effect, fine, but the phase exit does not depend on it.
...
- **No changes to the existing MCP tool signatures or CLI behavior.** Phase 2b is pure marketing + packaging — zero functional code changes to `src/stackly/`. Tests and CI remain green.
...
```
1. Stranger hears about Stackly via an HN post, a friend, or a Smithery search.
2. Clicks the link → lands on stackly.dev.
3. Watches the 60-second demo video embedded above the fold.
4. Clicks "View on GitHub" → lands on https://github.com/IdanG7/stackly.
...
6. On a fresh Windows 10/11 box with Debugging Tools installed:
     ...
     cd stackly
     uv sync
     uv run stackly doctor         # passes or prints clear guidance
     uv run stackly serve           # MCP server is up
   In another shell:
     uv run stackly fix --pid <crash_app.exe PID> --repo .
   → Claude Code opens with crash briefing preloaded.
7. Stranger tweets "wait, this actually works" → viral loop.
```

Phase exits when steps 1–6 work for a person who has never seen the codebase, and at least 4 MCP directory listings are live (step 1's "hears about Stackly" is backed by real discoverability surface).
